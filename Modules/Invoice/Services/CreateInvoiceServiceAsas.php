...
class CreateInvoiceServiceAsas
{
    ...
    public function execute(ContasReceber $contasReceber)
    {
        ...
        try {
            ...
            if (!$hasCreditCard) {
                $fatura = $this->criarFaturaModel($contasReceber, $contasReceber->vencimento, 1);
            ...
            $asasObject = $this->makeAsasCrediteInvoiceObject($contasReceber, $fatura->data_vencimento, $fatura);
            ...
            // if (!$hasCreditCard) {
            //     FaturaToHonorarioAsasJob::dispatch($fatura->fresh(), "HONORARIOS");
                // $this->asasService->downloadPdf($fatura->fresh());
            ...
            if( $contasReceber->payer->status_id!= 81){
    ...
    private function criarFaturaModel($contaReceber, string $competencia, $paymentMethodId)
    {
        ...
        $fatura = $contaReceber->payer->fatura()->create([
            ...
            'data_vencimento' =>Carbon::parse($competencia)->format('Y-m-20'),
            'status' => 'pendente',
            'subtotal' =>  $totalValor,
    ...
    private function makeAsasCrediteInvoiceObject($contaReceber, $dataVencimento, $fatura)
    {
        $totalValor = 0;
