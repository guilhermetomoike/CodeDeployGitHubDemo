...
use App\Http\Requests\BombeiroRequest;
use App\Http\Resources\BombeiroResource;

use App\Models\Empresa\Bombeiro;
use App\Services\Empresa\BombeiroService;
use Carbon\Carbon;
...
class BombeiroController extends Controller
{
    ...
    public function __construct(BombeiroService $bombeiroService)
    {
    ...
    {
        $alvaras = $this->bombeiroService->getAllBombeiros();
        if (!$alvaras) return new JsonResponse();
        return BombeiroResource::collection($alvaras);
    }

    public function store(BombeiroRequest $request)
    {
        $data = $request->validated();
        try {
            $alvara = $this->bombeiroService->storeBombeiro($data);
        } catch (InvalidArgumentException $e) {
            return response()->json(['message' => $e->getMessage()], 422);
        }

        return new BombeiroResource($alvara);
    }

    public function show(int $id)
    {
        $alvara = $this->bombeiroService->getBombeiro($id);
        return new BombeiroResource($alvara);
    }

    public function update(BombeiroRequest $request, Bombeiro $alvara)
    {
        $data = $request->validated();
        $this->bombeiroService->updateBombeiro($data, $alvara);
        return new BombeiroResource($alvara);
    }

    public function destroy(Bombeiro $alvara)
    {
        $this->bombeiroService->deleteBombeiro($alvara);
    ...
    {
        $alvaras =    Bombeiro::with(['empresa' => fn ($query) => $query->with('carteirasrel')])
            ->whereHas('empresa')
        ...
        $filename = 'Relatorio Bombeiro Medb.xlsx';

        $sheet->setCellValue('A1', 'Relatorio Bombeiro');
